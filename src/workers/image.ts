...
export async function runImageJob(payload: ImageJobPayload) {
  const { user_id: userId, application_id: appId, interaction_token: token } = payload;
  ...
  try {
    ...
    try {
      ...
      console.error('[image-worker] cache lookup error', cacheErr);
    }
    ...
    try {
      ...
      // cleanup old objects
      try {
        const { data: objects } = await supabase.storage.from('track-images').list(folder);
        ...
        console.error('[image-worker] cleanup error', cleanErr);
      }
    } catch (uploadErr) {
      console.error('[image-worker] image upload error', uploadErr);
    }
    ...
    (async () => {
      try {
        ...
        if (upErr) console.error('[image-worker] upsert error', upErr);
      } catch (e) {
        console.error('[image-worker] upsert threw', e);
      }
    })();
  } catch (err) {
